  ...
  class ApiClient
    ...
    def convert_to_type(data, return_type)
      ...
      when /\AHash\<String, (.+)\>\z/
        ...
        # models, e.g. Pet
        SpOrdersV0.const_get(return_type).build_from_hash(data)
      end
    end

    # Save response body into a file in (the defined) temporary folder, using
    # the filename from the "Content-Disposition" header if provided, otherwise
    # a random filename. The response body is written to the file in chunks in
    # order to handle files which size is larger than maximum Ruby String or
    # even larger than the maximum memory a Ruby process can use.
    #
    ...
    def download_file(request)
      ...
      request.on_headers do |response|
        ...
        tempfile =
          Tempfile.open(prefix, @config.temp_folder_path, encoding: encoding)
        @tempfile = tempfile
      end
      ...
      request.on_complete do |response|
        if tempfile
          tempfile.close
          @config
            .logger.info "Temp file written to #{tempfile.path}, please copy the file to a proper folder " \
