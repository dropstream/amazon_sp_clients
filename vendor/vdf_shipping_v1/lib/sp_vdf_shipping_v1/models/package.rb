    ...
    class Package
      ...
      def initialize(attributes = {})
        if (!attributes.is_a?(Hash))
          fail ArgumentError, "The input argument (attributes) must be a hash in `AmazonSpClients::SpVdfShippingV1::Package` initialize method"
        end
  
        # check to see if the attribute exists and convert string to symbol for hash key
        attributes = attributes.each_with_object({}) { |(k, v), h|
          if (!self.class.attribute_map.key?(k.to_sym))
            fail ArgumentError, "`#{k}` is not a valid attribute in `AmazonSpClients::SpVdfShippingV1::Package`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
          end
          h[k.to_sym] = v
        }
      ...
      def list_invalid_properties
        ...
        if @package_identifier.nil?
          invalid_properties.push('invalid value for "package_identifier", package_identifier cannot be nil.')
        end
      ...
      def valid?
        return false if @package_identifier.nil?
      ...
      def hash
        [package_identifier, tracking_number, manifest_id, manifest_date, ship_method, weight, dimensions].hash
      end
